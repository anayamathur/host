...
### ⚡ Absolute Zero Setup (Recommended for Beginners)

**Install once, type ONLY `raj-host` forever:**
```bash
curl -fsSL https://raw.githubusercontent.com/anayamathur/host/main/install-alias.sh | bash
source ~/.bashrc
```

**After this, you NEVER need curl again. Just run:**
```bash
raj-host
```

**What this does:**
- ✅ Creates a shell alias for `raj-host`
- ✅ Auto-downloads and runs latest version every time
- ✅ **Zero manual downloads** - ever!
- ✅ Just type `raj-host` - nothing else needed!

---

### 🔧 Standard Install (Auto-Update on Every Run)

**Install once, use forever - auto-updates on every run:**
```bash
curl -fsSL https://raw.githubusercontent.com/anayamathur/host/main/install.sh | sudo bash
```

**What this does:**
- ✅ Creates global `raj-host` command
- ✅ **Auto-downloads latest version** on every run
- ✅ **No manual curl/wget needed** ever again
- ✅ Just run `sudo raj-host` anytime - it handles everything!

**After installation, simply run:**
```bash
sudo raj-host
```
*That's it! It will automatically download the latest version and run.*

---

### 📦 Alternative: Package Manager Install

**For users who prefer traditional package installation:**
